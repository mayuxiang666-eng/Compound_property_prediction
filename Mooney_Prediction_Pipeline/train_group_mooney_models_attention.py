...
# ----------------------------------------------------
# PyTorch Soft Attention Ridge Regression
# ----------------------------------------------------
class AttentionRidgeRegression:
    def __init__(self, alpha_reg=100.0, alpha_attn=100.0, max_iter=1000, lr=0.01):
        ...
        self.coef_attn_ = None
        self.intercept_attn_ = None
    ...
    def fit(self, X, y, groups):
        ...
        # 3. Create padded arrays and masks
        X_padded = np.zeros((N_groups, max_size, D))
        group_mask = np.zeros((N_groups, max_size))
        ...
        mask_tensor = torch.tensor(group_mask, dtype=torch.float32)
        ...
        b_attn = torch.zeros(1, requires_grad=True)
        
        optimizer = optim.Adam([w, b, v, b_attn], lr=self.lr)
        
        for epoch in range(self.max_iter):
            optimizer.zero_grad()
            # 1. Compute attention logits: (N_groups, max_size)
            attn_logits = torch.matmul(X_tensor, v) + b_attn
            # 2. Masked Softmax (set padded slots to -1e9)
            masked_logits = attn_logits.masked_fill(mask_tensor == 0.0, -1e9)
            weights = torch.softmax(masked_logits, dim=1) # (N_groups, max_size)
            # 3. Weighted features: (N_groups, D)
            X_weighted = torch.sum(X_tensor * weights.unsqueeze(-1), dim=1)
            # 4. Predict
            preds = torch.matmul(X_weighted, w) + b
            # 5. Loss: MSE + L2 regularization on both weight vectors
            loss = torch.mean((preds - y_tensor.squeeze(-1)) ** 2) + \
                   (self.alpha_reg / N_groups) * torch.sum(w ** 2) + \
                   (self.alpha_attn / N_groups) * torch.sum(v ** 2)
            loss.backward()
            optimizer.step()
            
        self.coef_ = w.detach().numpy()
        self.intercept_ = b.detach().numpy()[0]
        self.coef_attn_ = v.detach().numpy()
        self.intercept_attn_ = b_attn.detach().numpy()[0]
        
    def predict(self, X):
        # When predicting on test/validation centroids, we don't have multiple candidate batches
        # So we just compute standard linear prediction on the centroid features
        X_scaled = self.scaler.transform(X)
        return np.dot(X_scaled, self.coef_) + self.intercept_


class TwoStageModel:
    def __init__(self, recipe_cols, process_cols, alpha_reg=100.0, alpha_attn=100.0, max_iter=1000, lr=0.01):
        self.recipe_cols = recipe_cols
        self.process_cols = process_cols
        self.baseline_model = Stage1Baseline(recipe_cols)
        self.dev_transformer = PhysicsDeviationTransformer(recipe_cols, process_cols)
        self.residual_model = AttentionRidgeRegression(alpha_reg=alpha_reg, alpha_attn=alpha_attn, max_iter=max_iter, lr=lr)
...
def main():
    ...
    # ----------------------------------------------------
    # 5. Split and Train Two-Stage Group Models with Soft Attention
    # ----------------------------------------------------
    print("\n=== STEP 5: TRAINING SEPARATED TWO-STAGE GROUP MODELS WITH SOFT ATTENTION ===")
    ...
    def run_two_stage_modeling(df_track_unagg, track_name, output_dir, track_recipe_cols, track_process_cols):
        ...
        print(f"\n--- Training Two-Stage Group Model (Soft Attention) for {track_name} ({len(df_groups_metadata)} groups, {len(df_track_unagg)} individual batches) ---")
        ...
        for fold, (train_idx, val_idx) in enumerate(gkf.split(df_groups_metadata, df_groups_metadata['MNY'], df_groups_metadata['OrderID'])):
            ...
            # --- Attention Ridge regression for Fold ---
            ts_model = TwoStageModel(available_recipe, available_process, alpha_reg=100.0, alpha_attn=100.0)
        ...
        print(f"==================== TWO-STAGE MODEL CV REPORT ({track_name} - Attention) ====================")
        ...
        final_ts_model = TwoStageModel(available_recipe, available_process, alpha_reg=100.0, alpha_attn=100.0)
        ...
        with open(os.path.join(output_dir, "mny_predictive_modeling_report.md"), 'w', encoding='utf-8') as f:
            f.write(f"# Two-Stage Group Predictive Modeling Report (Soft Attention) - {track_name}\n\n")
            f.write(f"This model uses a Two-Stage architecture with PyTorch Soft Attention optimization to decouple static recipe baseline and dynamic process variations.\n\n")
    ...
    for track_name, is_oil, is_silica, folder_name in sub_tracks:
        ...
        r2, rmse, dev_corr = run_two_stage_modeling(
            df_sub, 
            track_name, 
            os.path.join(WORKSPACE_DIR, "Mooney_Prediction_Pipeline", "models", folder_name, "group_model_attention"),
            recipe_cols,
            core_process_features
        )
    ...
    print("\n" + "="*60)
    print("      ALL TWO-STAGE GROUP ATTENTION MODELS TRAINED SUCCESSFULLY")
    print("="*60)
