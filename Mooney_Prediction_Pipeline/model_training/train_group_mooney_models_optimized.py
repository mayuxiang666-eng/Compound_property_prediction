...
# ----------------------------------------------------
# 1. Optimized Two-Stage Modeling with Scaling & Bias Offsetting
# ----------------------------------------------------

class Stage1Baseline:
    """
    Optimized Stage 1 Baseline model.
    Instead of using static compound average lookup, it fits a Ridge regression model
    incorporating recipe and raw material lot features (like supplier_rubber_viscosity_avg)
    to dynamically offset predictions on lot shifts, then applies a compound-level residual bias.
    """
    ...
    def fit(self, X_train_full, y_train):
        ...
        # Step 1: Fit regression model on recipe features (which include raw rubber supplier viscosity)
        X_rec = X_train_full[self.recipe_cols]
        self.fallback_model.fit(X_rec, y_train)
        
        # Step 2: Calculate baseline predictions and compute residual biases for each compound
...
class TwoStageModel:
    def __init__(self, recipe_cols, process_cols, residual_model):
        ...
        self.residual_model = make_pipeline(SimpleImputer(strategy='median'), StandardScaler(), residual_model)
...
def main():
    ...
    # ----------------------------------------------------
    # Safe-Net Pre-Filtering for Sensor Glitches
    # ----------------------------------------------------
    print("\nApplying batch-level Safe-Net filters on raw curves...")
    initial_count = len(df_seg)
    
    # Filter 1: Power sensor saturation (Max peak power should not exceed 65,000 kW)
    if 'phys_peak_power' in df_seg.columns:
        df_seg = df_seg[df_seg['phys_peak_power'] < 65000.0]
    if 'Stage1_power_Max' in df_seg.columns:
        df_seg = df_seg[df_seg['Stage1_power_Max'] < 65000.0]
        
    # Filter 2: Temperature flatlines (Zero or near-zero variance)
    if 'phys_temp_change_rate_std' in df_seg.columns:
        df_seg = df_seg[df_seg['phys_temp_change_rate_std'] > 0.01]
        
    print(f"Safe-Net filter: Retained {len(df_seg)} of {initial_count} batches ({initial_count - len(df_seg)} anomalous batches discarded).")
    ...
    # Group Aggregation
    # ----------------------------------------------------
    print("\n=== STEP 3: RUNNING GROUP LEVEL AGGREGATION ===")
    df_seg['MNY'] = pd.to_numeric(df_seg['MNY'], errors='coerce')
    df_seg = df_seg.dropna(subset=['MNY'])
    print(f"Valid MNY rows for aggregation: {len(df_seg)}")
    ...
    # Core process features to keep (added suggestions for cold starts, heat history and integration)
    core_process_features = [
        ...
        # Added dynamic process features
    ...
    out_path_raw = os.path.join(WORKSPACE_DIR, "Mooney_Prediction_Pipeline", "models", "hefei_group_training_dataset.csv")
    df_group.to_csv(out_path_raw, index=False)
    print(f"Saved raw group dataset of {len(df_group)} rows to: {out_path_raw}")
    ...
    df_clean = df_group[preds == 1].copy()
    ...
    print(f"Cleaned group dataset size: {len(df_clean)} rows.")
    print("=============================================================================")

    out_path_clean = os.path.join(WORKSPACE_DIR, "Mooney_Prediction_Pipeline", "models", "hefei_group_training_dataset_clean.csv")
    df_clean.to_csv(out_path_clean, index=False)
    print(f"Saved cleaned group dataset to: {out_path_clean}")

    # ----------------------------------------------------
    # 5. Split and Train Two-Stage Group Models
    # ----------------------------------------------------
    print("\n=== STEP 5: TRAINING SEPARATED TWO-STAGE GROUP MODELS ===")

    def run_two_stage_modeling(df_track, track_name, output_dir, track_recipe_cols, track_process_cols):
        os.makedirs(output_dir, exist_ok=True)
        print(f"\n--- Training Two-Stage Group Model for {track_name} ({len(df_track)} samples) ---")
        
        df_track = df_track.rename(columns={col: clean_feature_name(col) for col in df_track.columns})
        
        available_recipe = [r for r in track_recipe_cols if r in df_track.columns]
        available_process = [p for p in track_process_cols if p in df_track.columns]
        
        X = df_track.copy()
        y = df_track['MNY']
        groups = df_track['OrderID']
        
        gkf = GroupKFold(n_splits=5)
        oof_preds = np.zeros(len(df_track))
        
        for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups)):
            X_train, y_train = X.iloc[train_idx], y.iloc[train_idx]
            X_val, y_val = X.iloc[val_idx], y.iloc[val_idx]
            
            res_model = Ridge(alpha=100.0)
            ts_model = TwoStageModel(available_recipe, available_process, res_model)
            ts_model.fit(X_train, y_train)
            oof_preds[val_idx] = ts_model.predict(X_val)
        ...
        final_res_model = Ridge(alpha=100.0)
        final_ts_model = TwoStageModel(available_recipe, available_process, final_res_model)
        ...
        with open(os.path.join(output_dir, "mny_predictive_modeling_report.md"), 'w', encoding='utf-8') as f:
            f.write(f"# Two-Stage Group Predictive Modeling Report - {track_name}\n\n")
            f.write(f"This model uses a Two-Stage architecture to decouple static recipe baseline and dynamic process variations.\n\n")
    ...
    df_clean['is_silica_system'] = ((df_clean['silica_phr'] >= 25.0) & (df_clean['weight_pct_silian'] > 0.0)).astype(float)
    ...
    for track_name, is_oil, is_silica, folder_name in sub_tracks:
        df_sub = df_clean[(df_clean['is_oil_loading_present'] == is_oil) & (df_clean['is_silica_system'] == is_silica)].copy()
        if len(df_sub) < 40:
            print(f"\n[Warning] Too few sub-track samples ({len(df_sub)}). Falling back to parent track.")
            df_sub = df_clean[df_clean['is_oil_loading_present'] == is_oil].copy()
            track_name = "With-Oil Fallback" if is_oil == 1.0 else "Without-Oil Fallback"
            
        r2, rmse, dev_corr = run_two_stage_modeling(
            df_sub, 
            track_name, 
            os.path.join(WORKSPACE_DIR, "Mooney_Prediction_Pipeline", "models", folder_name, "group_model"),
            recipe_cols,
            core_process_features
        )
    ...
    print("\n" + "="*60)
    print("      ALL OPTIMIZED TWO-STAGE GROUP MODELS TRAINED SUCCESSFULLY")
    print("="*60)
