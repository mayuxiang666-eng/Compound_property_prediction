...
# ----------------------------------------------------
# 1. Classes for Two-Stage Modeling with Non-Linear Stage 2
# ----------------------------------------------------

class Stage1Baseline:
    """
    Stage 1 Baseline model using Ridge regression to model recipe and raw lot properties
    (like supplier_rubber_viscosity_avg) dynamically, followed by a compound-level bias.
    """
    ...
    def fit(self, X_train_full, y_train):
        ...
        # Step 1: Fit regression model on recipe features
...
class TwoStageNonLinearModel:
    """
    Two-Stage Architecture incorporating a non-linear Stage 2 model (LightGBM).
    This allows capturing non-linear interactions between process variations and final viscosity.
    """
...
def main():
    ...
    # ----------------------------------------------------
    # 5. Split and Train Two-Stage Group Models (Non-Linear)
    # ----------------------------------------------------
    print("\n=== STEP 5: TRAINING SEPARATED TWO-STAGE NON-LINEAR GROUP MODELS ===")

    def run_two_stage_modeling(df_track, track_name, output_dir, track_recipe_cols, track_process_cols):
        os.makedirs(output_dir, exist_ok=True)
        print(f"\n--- Training Two-Stage Non-Linear Group Model for {track_name} ({len(df_track)} samples) ---")
        ...
        for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups)):
            ...
            # Non-linear residual learning using LightGBM with robust defaults
        ...
        final_ts_model = TwoStageNonLinearModel(available_recipe, available_process, final_res_model)
        ...
        with open(os.path.join(output_dir, "mny_predictive_modeling_report.md"), 'w', encoding='utf-8') as f:
            f.write(f"# Two-Stage Non-Linear Group Predictive Modeling Report - {track_name}\n\n")
            f.write(f"This model uses a Two-Stage Non-Linear architecture with LightGBM in Stage 2 to decouple recipe baselines and capture complex process variations.\n\n")
    ...
    for track_name, is_oil, is_silica, folder_name in sub_tracks:
        ...
        r2, rmse, dev_corr = run_two_stage_modeling(
            df_sub, 
            track_name, 
            os.path.join(WORKSPACE_DIR, "Mooney_Prediction_Pipeline", "models", folder_name, "group_model_nonlinear"),
            recipe_cols,
            core_process_features
        )
    ...
    print("\n" + "="*60)
    print("      ALL OPTIMIZED NON-LINEAR MODELS TRAINED SUCCESSFULLY")
    print("="*60)
