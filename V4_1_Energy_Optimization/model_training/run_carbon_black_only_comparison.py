# ============================================================================
# Carbon Black ONLY Compound Prediction Performance Audit Runner
# ============================================================================
# Filters test set exclusively for Carbon Black compounds according to plant rules
# (A- / B- / R- / C- prefix compounds) and exports side-by-side performance table.
#
# Generates:
# - reports/v36_explainable_production/carbon_black_only_comparison_report.csv
# ============================================================================
...
def run_cb_only_audit():
    print("=" * 90)
    print("  CARBON BLACK ONLY COMPOUND PERFORMANCE COMPARISON RUNNER")
    print("=" * 90)
    ...
    # Filter EXCLUSIVELY for Carbon Black compounds
    df_cb_fix = df_te_fix[df_te_fix['material_system'] == 'CarbonBlack'].copy()
    cb_compounds = df_cb_fix['CleanCompound'].value_counts()

    rows = []
    for cmp in cb_compounds.index:
        ...
        sys_leg = sub_leg['material_system'].iloc[0] if not sub_leg.empty else 'Unknown'
        ...
        if mae_fix <= 2.0:
            diag_zh = "优秀：精度极高，CB 3-Expert + EWMA 完美匹配"
        elif mae_fix <= 3.0 and dir_fix >= 50.0:
            diag_zh = "良好：炭黑结构分散与 Bottom 扭矩响应捕获良好"
        elif sys_leg != 'CarbonBlack':
            diag_zh = "纠偏受益：剔除误匹配 PID 偶联逻辑，MAE 显着改善"
        elif mae_fix > 4.0:
            diag_zh = "偏高：受生胶/炭黑 Raw COA 跨订单批次跳变影响"
        else:
    ...
    comp_df.to_csv(os.path.join(out_dir, 'carbon_black_only_comparison_report.csv'), index=False, encoding='utf-8-sig')

    print("=" * 115)
    print("      CARBON BLACK ONLY COMPOUND PERFORMANCE COMPARISON REPORT")
    print("=" * 115)
    ...
    print(f"\nCarbon Black only report saved to: {os.path.join(out_dir, 'carbon_black_only_comparison_report.csv')}\n")
    return comp_df
