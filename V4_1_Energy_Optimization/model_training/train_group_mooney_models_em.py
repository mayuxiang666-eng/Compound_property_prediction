...
def main():
    ...
    # ----------------------------------------------------
    # 5. Split and Train Two-Stage Group Models with EM
    # ----------------------------------------------------
    print("\n=== STEP 5: TRAINING SEPARATED TWO-STAGE GROUP MODELS WITH HARD EM ===")

    def run_two_stage_modeling(df_track_unagg, track_name, output_dir, track_recipe_cols, track_process_cols, max_em_iter=10):
        ...
        print(f"\n--- Training Two-Stage Group Model (EM) for {track_name} ({len(df_groups_metadata)} groups, {len(df_track_unagg)} individual batches) ---")
        ...
        for fold, (train_idx, val_idx) in enumerate(gkf.split(df_groups_metadata, df_groups_metadata['MNY'], df_groups_metadata['OrderID'])):
            ...
            # --- Hard EM Algorithm for Fold ---
            # 1. Initialize by fitting on centroids of train groups
            df_train_centroid = get_centroid(df_train_unagg)
            
            res_model = Ridge(alpha=100.0)
            ts_model = TwoStageModel(available_recipe, available_process, res_model)
            ts_model.fit(df_train_centroid, df_train_centroid['MNY'])
            
            prev_selected_indices = None
            for em_iter in range(max_em_iter):
                preds = ts_model.predict(df_train_unagg)
                abs_errors = np.abs(preds - df_train_unagg['MNY'])
                df_train_unagg['em_error'] = abs_errors
                
                # Select the batch with minimum absolute error for each group
                best_indices = df_train_unagg.groupby(['OrderID', 'PalletID'])['em_error'].idxmin()
                
                # Check convergence
                if prev_selected_indices is not None and set(best_indices) == set(prev_selected_indices):
                    break
                prev_selected_indices = best_indices
                
                # Re-fit model on selected batches
                df_train_selected = df_train_unagg.loc[best_indices]
                ts_model.fit(df_train_selected, df_train_selected['MNY'])
            
            # 2. Predict on centroid of validation groups
        ...
        print(f"==================== TWO-STAGE MODEL CV REPORT ({track_name} - EM) ====================")
        ...
        # Fit final model on all clean training data using EM
        df_all_centroid = get_centroid(df_track_unagg)
        final_res_model = Ridge(alpha=100.0)
        final_ts_model = TwoStageModel(available_recipe, available_process, final_res_model)
        final_ts_model.fit(df_all_centroid, df_all_centroid['MNY'])
        
        prev_selected_indices = None
        for em_iter in range(max_em_iter):
            preds = final_ts_model.predict(df_track_unagg)
            abs_errors = np.abs(preds - df_track_unagg['MNY'])
            df_track_unagg['em_error'] = abs_errors
            
            best_indices = df_track_unagg.groupby(['OrderID', 'PalletID'])['em_error'].idxmin()
            if prev_selected_indices is not None and set(best_indices) == set(prev_selected_indices):
                print(f"Final model EM converged at iteration {em_iter}")
                break
            prev_selected_indices = best_indices
            
            df_track_selected = df_track_unagg.loc[best_indices]
            final_ts_model.fit(df_track_selected, df_track_selected['MNY'])
        ...
        with open(os.path.join(output_dir, "mny_predictive_modeling_report.md"), 'w', encoding='utf-8') as f:
            f.write(f"# Two-Stage Group Predictive Modeling Report (EM) - {track_name}\n\n")
            f.write(f"This model uses a Two-Stage architecture with EM-based instance selection to decouple static recipe baseline and dynamic process variations.\n\n")
    ...
    for track_name, is_oil, is_silica, folder_name in sub_tracks:
        ...
        r2, rmse, dev_corr = run_two_stage_modeling(
            df_sub, 
            track_name, 
            os.path.join(WORKSPACE_DIR, "Mooney_Prediction_Pipeline", "models", folder_name, "group_model_em"),
            recipe_cols,
            core_process_features
        )
    ...
    print("\n" + "="*60)
    print("      ALL TWO-STAGE GROUP EM MODELS TRAINED SUCCESSFULLY")
    print("="*60)
