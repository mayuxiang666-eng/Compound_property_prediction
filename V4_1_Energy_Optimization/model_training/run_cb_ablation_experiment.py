...
def run_cb_ablation_experiment():
    ...
    configs = {
        'CB_A0 (Existing CB V1)': {'s2_cols': list(set(s2_cols_base + v1_feat_cols)), 'use_subsystem': True},
        'CB_A1 (V2 Features Only)': {'s2_cols': list(set(v2_feat_cols)), 'use_subsystem': False},
        'CB_A2 (V2 + Dispersion Expert)': {'s2_cols': list(set(v2_feat_cols + ['Stage2_DryMixing_Duration', 'Stage2_DryMixing_power_Mean'])), 'use_subsystem': True},
        'CB_A3 (V2 + Dispersion + Bottom Expert)': {'s2_cols': list(set(s2_cols_base + v2_feat_cols)), 'use_subsystem': True},
        'CB_A4 (Full CB Subsystem V2)': {'s2_cols': list(set(s2_cols_base + v2_feat_cols)), 'use_subsystem': True},
    }
    ...
    for cmp, c_df in df_te.groupby('CleanCompound'):
        if len(c_df) >= 3:
            ...
            c_indices = c_df['test_idx'].values
            c_p_v1 = preds_dict['CB_A0 (Existing CB V1)'][c_indices]
            c_p_v2 = preds_dict['CB_A4 (Full CB Subsystem V2)'][c_indices]
