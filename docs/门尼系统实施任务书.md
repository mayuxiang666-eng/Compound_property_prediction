...
| 日期 | **2026-08-05**（库表/托盘定稿）；**2026-08-05 修订**：按 HybridUnified 实码对齐架构说明与评审 P0 控制点（增补，不推倒前文） |
...
[Windows Server]
  ...
  SQL Server: MixingCurveData
       ...
       ├─ MooneyPredictMiss   ← 漏预测/拒写审计（不进结果主表）
...
| 2 | **成功才 INSERT** `MooneyResult`；失败**不写半截行**，须写 `MooneyPredictMiss`；`Final`/`Raw` NOT NULL |
...
### 0.5 推荐上线路径（评审后增补；不替代分版清单）

```text
阶段1 V3.7 影子预测：真写 MooneyResult + Miss；验证就绪门/覆盖率/延迟（不以「2s 出数」验收）
阶段2 V3.8 影子校准：组级误差与 Queue 全跑；默认不自动写 BiasOffset（或 Status=SHADOW）
阶段3 回放门禁通过后：白名单胶料有限自动写 State
阶段4 补齐 Trust/KPI 契约后做 V4.0
```

当前状态：**有条件通过方案评审，不通过生产自动校准上线**——须先完成 §10 工程控制点。

---

## 1. 代码基线（对照仓库）

### 1.0 实码架构（包内 / 包外边界）— 2026-08-05 增补

> 依据：`hybrid_unified_model.py`、`bias_shrinkage.py`、`label_group_handler.py`、`stage3_online_calibration.py`、`save_production_v37_model_package.py`。  
> **工程契约不得与下列公式冲突；冲突时以实码为准并回改文档。**

```mermaid
flowchart TB
  MMS["MMS SFEPLANT"] --> FB["FeatureBuilder 包外"]
  FB -->|"s1∪s2 fillna(0) 与训练一致"| HU["HybridUnified 包内"]
  HU --> Raw["Raw = S1 + S1b + S2"]
  ST["SQL MooneyCalibrationState"] --> Fin["Final = Raw + offset"]
  Raw --> Fin
  Lab["Lab 延迟"] --> Q["Queue 同 LabelGroup 一次"]
  Q --> Eng["EWMA 更新 State"]
  Eng --> ST
```
...
| 层 | 实码行为 | 生产含义 |
|----|----------|----------|
| **包内 Hybrid** | `predict` → `(Raw, s1, s1b, s2)`；`Raw = S1 + S1b(CompoundName) + S2(route)` | 不读 Lab；不用 `_label_group_id`；**不含 Stage3** |
| **路由** | `route_key = (material_system, phase_route)`；phase 见 §1.2 | 与打包 `use_material_route_matrix=True` 一致 |
| **LabelGroup** | 训练 `_sample_weight=1/size`；评估常 `mean(pred)` vs 单 Lab | **不是** Hybrid 输入；用于损失/评估与生产「同 Lab 校准一次」 |
| **Stage3 类** | `e = Lab − Raw`；默认按 **CompoundName** 时间序；α 实码约 0.65/0.20；`lag_k=3` | **在包外**；joblib calibrator 无持久历史 |
| **打包 stub** | 仍可能 `calibrate_time_series(..., MNY)` | **禁止上线**；必须改读 SQL State |
| **填 0** | 训练/打包对 s1∪s2 `fillna(0.0)` | 生产**保持填 0**以对齐分布；拒写靠曲线就绪门 + Miss，不另造 median 插补 |

生产 State 键默认 **`CompName|Mixer`**（机台漂移工程选择）。与 Stage3 类默认仅 `CompoundName` **有意不同**：影子回放若要对齐 Python 类可用 CompoundName；写库 State 仍用 `CompName|Mixer`。生产 α 表见 §6.6（英文扩展表）；与 `Stage3DelayedFeedbackCalibrator` 的 0.65/0.20 **不同，须在文档与配置中标明**。
...
RawPrediction      = Stage1Pred + Stage1bBias + Stage2Delta
                   （HybridUnifiedMooneyModel.predict 的 uncal；包内不含 Stage3）
...
禁止：当前车 Lab 参与当次 Final（禁止 stub calibrate_time_series 喂当前 MNY）
允许：历史 CONSUMED 更新 State → 只服务后续车

组级校准误差（V3.8；对齐评估 mean(pred) vs 单 Lab；禁止代表车）：
  GroupRawError    = LabActualMny - mean(RawPrediction_i)   # 同 LabelGroupId
  GroupOnlineError = LabActualMny - mean(FinalPrediction_i)
  EWMA 只吃 GroupRawError 一次；各车行级 RawError/OnlineError 仅展示
```
...
| 账号 | 允许 | 禁止 |
|------|------|------|
| `mooney_predict` | INSERT `MooneyResult` / `MooneyPredictMiss`；SELECT Model/State | UPDATE/DELETE 预测列 |
...
| Pipeline | 版本 | 周期 | 配置建议 |
|----------|------|------|----------|
| A 预测 | V3.7+ | **2s 轮询** | `PREDICT_POLL_SECONDS=2`（**不是**端到端 SLA，见 §10） |
...
**可预测（与 `extract_features_for_row` 对齐，硬门）：** BatchCurve 有行，Curve1、Curve2 非空，**且解码后点数 ≥ 10**。不满足 → 不写 `MooneyResult`，写 `MooneyPredictMiss`（如 `CURVE_INCOMPLETE`）。  
**过滤（可配）：** CompName `M1-%`/`R1-%`；OrderID 不含 `H`。  
**ProductTime：** 优先抄同车 MixingInfo；仅 XML `Time` 同源解析时才 +8；MMS 已是本地时禁止再 +8。
...
落盘：`sql/mooney_ddl.sql`（五表 + `MooneyPredictMiss` + 权限 + 初始 Model 一行）。  
...
| 校准 | Queue **每组一行**；组级误差用 **mean(Raw)**（§2.2）；已 CONSUMED 不再 NEW |
...
**过滤唯一（必须）：** 至多一行 `IsActive=1`（`CREATE UNIQUE INDEX ... WHERE IsActive=1`）。  
初始一行 Active。
...
| Status | ACTIVE / STALE / **SHADOW**（影子期不参与 Predict） |
| LastConsumedGroupRawError | FLOAT NULL；变点 Δe 用 |
...
| LabelGroupId | 组级 |
| LabSourceKey | NVARCHAR；如 `OrderID\|PalletID\|LabResultTime`；**UNIQUE** |
| OrderID, BatchID | 追溯用；**误差不用代表车** |
| CompName, Mixer, PalletID | |
| LabActualMny, LabResultTime | |
| GroupRawMean, GroupFinalMean | 组内 mean(Raw)/mean(Final) |
| GroupRawError, GroupOnlineError | Lab−均值；**EWMA 只用 GroupRawError** |
| MatchQuality | |
| FeedbackStatus | NEW / CONSUMED / REJECTED / SKIPPED_DUP |
| RejectReason, MatchedTime | |

**库级幂等（必须）：** `UNIQUE(LabSourceKey)`；过滤唯一索引 `LabelGroupId` WHERE `FeedbackStatus IN ('NEW','CONSUMED')`。

### 3.5 `MooneyCalibrationEventLog`

追加：`EventId, ScopeKey, FeedbackId, PreviousBiasOffset, NewBiasOffset, GroupRawError, AlphaUsed, ChangePointDetected, UpdateReason, UpdateTime`。

### 3.6 `MooneyPredictMiss`（V3.7+ 增补）

| 列 | 说明 |
|----|------|
| MissId | BIGINT IDENTITY PK |
| OrderID, BatchID | |
| MissTime | DATETIME2 |
| ReasonCode | `CURVE_INCOMPLETE` / `FEATURE_GATE` / `INFER_ERROR` / … |
| Detail | NVARCHAR(512) |
| ResolvedAt | DATETIME2 NULL |

### 3.7 可选 `MooneyHeartBeat`

`ServiceName, LastBeatUtc, Detail` — Health 用；V3.7 建议建。  
Health 还须：最近成功 INSERT 年龄、近 1h Miss 率。

### 3.8 V4.0 `MooneyKpiDaily`（后加）
...
| `miss_writer.py` | V3.7 | INSERT `MooneyPredictMiss` | 写成伪成功 Result |
...
4. PID/CB 代理特征；cluster；缺列填 **0.0**（**与 V3.7 训练/打包一致**，禁止擅自改成 median）  
5. 组装 LabelGroupId、PhaseRoute、BatchID 字符串规则  
6. 硬门：Curve1/2 非空且点数 ≥10，否则 Miss、不进推理  
...
4. 未过就绪门 / 特征或推理失败 → INSERT MooneyPredictMiss，**不 INSERT Result**，继续
5. 成功 → INSERT MooneyResult（LabStatus=PENDING；精选特征一并写入）
6. PK 冲突 → 当作已有，跳过
7. sleep PREDICT_POLL_SECONDS（轮询周期 ≠ 端到端 SLA）
```

### 6.5 LabMatcher（V3.8）

```text
1. 拉 Lab（order/pallet/mny/time）→ 生成 LabSourceKey
2. 拼 LabelGroupId；UPDATE 组内所有行 Lab* + 各车行级 errors
3. 计算 GroupRawMean / GroupRawError（Lab − mean(Raw)）；禁止用单车代表误差做校准
4. NOPALLET → 可展示；Queue REJECTED/NOPALLET_NO_CALIB，不校准
5. LabSourceKey 或 LabelGroup 已 NEW/CONSUMED → SKIPPED_DUP
6. 否则 INSERT Queue NEW（带组级误差列）
7. Lab 复测：若组已 CONSUMED → 不二次校准；展示列更新须带 LabRevision 标记（可选）
```

### 6.6 CalibrationEngine（V3.8）

`calibration_alpha.json`（**生产策略表**，相对 `Stage3DelayedFeedbackCalibrator` 的 0.65/0.20 有意扩展）：  
变点 0.60 / 低频 0.40 / 默认 0.25 / 高频 0.15 / 拒绝 0；  
变点：`Δe = GroupRawError − LastConsumedGroupRawError`，`|Δe|≥2.0`；  
单次 `|new_bias−old_bias|≤1.5`，再全局 clip ±4。

```text
单事务：
  SELECT State WITH (UPDLOCK, ROWLOCK)
  → 质检 → VALIDATED
  → new_bias = (1-α)*old + α*GroupRawError
  → UPDATE State（含 LastConsumedGroupRawError）；INSERT Event
  → Queue=CONSUMED
影子模式：只写 Event / Status=SHADOW，Predict 仍读 Bias=0 或旧 ACTIVE
...
| # | 做什么 | 验收 |
|---|--------|------|
| 1 | `sql/mooney_ddl.sql`（含 Miss、权限、Model Active 唯一） | SSMS 可见 |
| 2 | 打包 + 改 predict_online 读 SQL offset | 无当前 Lab 也能出 Final |
| 3 | FeatureBuilder + PredictService + NSSM | 成功有 Result；失败有 Miss；恒等式成立 |
| 4 | ASP.NET Recent/详情/curve/features/Health | Health 含 INSERT 年龄与 Miss 率 |
| 5 | Vue 监控+详情 | Network 无 Python |
| 6 | IIS | 刷新不 404 |

### 7.2 V3.8 — Lab + 校准（默认先影子写 State）

| # | 做什么 | 验收 |
|---|--------|------|
| 1 | Datamart Lab 源 | 能打样例 |
| 2 | Matcher：组 mean(Raw) + 幂等 Queue | 同盘 Lab 一致；Final 未变；无双 CONSUMED |
| 3 | Engine：UPDLOCK 事务；影子模式 | Event 有行；**自动 Bias 默认关闭**直至回放门禁 |
| 4 | NOPALLET | 不更新 State |
| 5 | API/UI Lab+校准页 | 状态机可见 |

### 7.3 V4.0 — Trust + KPI

Trust 列 + KPI 页；回归 V3.7/3.8；依赖阶段3有限自动校准或明确 n/a。

---

## 8. 验收剧本（端到端）

1. 曲线就绪且点数≥10 → 出现 `MooneyResult`；`Final = Raw + Offset`（记录就绪→写入延迟，**不以 2s 轮询当 SLA**）  
2. 点数不足 / 推理失败 → **无 Result**，有 `MooneyPredictMiss`  
3. 同 Pallet 3 车 → Lab 回填 3 行；Queue/Event **1** 次；GroupRawError = Lab−mean(Raw)  
4. NOPALLET 车有 Lab → 展示误差；State 不变  
5. 自动校准开启后：新车 Offset 可变；**旧车 Final 不变**  
6. 双 Predict 实例同车 → 仅一行（PK）  
7. 只读账号无法 UPDATE Final  
8. 仿真回放门禁：窗口 `MAE(|online|) ≤ MAE(|raw|)`（见英文规范修订）  
...
## 10. 工程控制点（评审 P0/P1 增补；上线自动校准前必须满足）

| ID | 控制点 | 契约要点 |
|----|--------|----------|
| P0-1 | 组级误差 | `GroupRawError = Lab − mean(Raw)`；禁止代表车 |
| P0-2 | Lab 幂等 | `LabSourceKey` UNIQUE + LabelGroup NEW/CONSUMED 过滤唯一 |
| P0-3 | State 并发 | `UPDLOCK, ROWLOCK` 单事务更新 State+Event+Queue |
| P0-4 | 仿真门禁 | 窗口 `MAE\|online\| ≤ MAE\|raw\|`（禁止有符号 online≤raw） |
| P0-5 | 填 0 | **保持**与训练一致；拒写靠曲线硬门 + Miss，不改 imputer |
| P0-6 | 漏预测可观测 | 必须有 `MooneyPredictMiss` |
| P0-7 | SLA | 2s 仅为轮询；验收用就绪→写入延迟分位数 |
| P0-8 | 曲线完成 | Curve1/2 非空 **且** 点数≥10（对齐 `extract_features_for_row`） |
| P1 | Active 唯一 / 心跳增强 / 单次 Δbias≤1.5 / 复测不二次 CONSUMED / Scope 与 Stage3 CompoundName 差异写清 | 见 §1.0、§3、§6.6 |
...
门尼 `ProductTime` 与同车 MixingInfo **同一习惯**；优先抄 MixingInfo，避免对已是本地时的 MMS 字段再 +8。
...
| 失败写红灯行 | **失败不写行** + **MooneyPredictMiss** |
| snake_case 表列 | 与 MixingCurve 对齐的 **PascalCase** |
| 英文规范表名 | 工程以本文 `Mooney*` 为准 |

## 附录 C. 2026-08-05 评审对齐修订摘要（在原文上增补）

- 增补 §1.0 实码架构（包内 Raw / 包外 Stage3 / LabelGroup 三义）  
- 增补 §0.5 影子→自动路径；§10 P0 控制点  
- 校准改为组级 mean(Raw)；Queue 幂等与 State 行锁  
- 填 0 明确与训练一致；可预测硬门对齐特征脚本  
- 英文 Simulation Gate 改为绝对误差窗口 MAE（见英文规范修订段）  
